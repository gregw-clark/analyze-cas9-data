...
def InitClean(df):
	df=df[(df.go_screened > 0)]							##must have g0 by default								
	df=df[(df.secondary_allele == "Deletion") | (df.primary_allele == "Deletion")]	##looking for deletions only
	df=df[(df.experimental) != "t"]							##exclude experimental data
	...
	###Write out data to local as intermediary step
	df.to_csv("./AllImits.nan.csv",sep=";")##Avoid data collision - we have some comma-separated columns
	#return df
...
def AggregateExp(filename):
	##'filename' is a semi-colon delimited csv file.
	##Function expects 'cleaned data' from a local csv file 'filename' 
	...
	header.append('DeletionSize')	#Added column: bp size of intended deletions
	header.append('DeletionType')	#Added column: 2 gRNA, 4 gRNA or other
	header.append('AggregExp')	#Added column: was the experiment an aggregate
	io.pop(0)	#remove header line
	experiments={}
	Uexperiments={}
...
#	maindat.append(header)
	...
	df.to_pickle('Pared.DataFrame.nan.pkl')
...
if __name__ == "__main__":
	...
	InitClean(data)
	#grnaID('CleanImits.nan.csv')
	#AggregateExp('ImitsPared.nan.csv')
