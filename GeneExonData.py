...
def InitClean(df):
	...
	df=df[(df.secondary_allele == "Deletion") | (df.primary_allele == "Deletion")]
	...
	df.to_csv("./CleanImits.csv",sep=";")##We already have comma-separated columns
	#return df
...
def grnaID(filename):
	...
	for r in Ic:
		...
		if not gene_info:
			...
			finalseq=[]
			finalcut=[]
	#		if marker_symbol == 'Cbr3':
	#			print gMGI_ID,seq_positions,valids,grna_ids
			for v in valids:
				mS,mC=v
				vS=reverseSeq(mS)
				if mS in grna_seqs:
...
def AggregateExp(filename):
	...
	header.append('UpstreamSites')
	header.append('DownstreamSites')
	...
	for line in io:
		...
		cutS,leftF,rightF=cutSizeDet(cut_sites)	
		_line=[]
...
#	maindat.append(header)
	...
	print df
	df.to_pickle('Sites.DataFrame.pkl')



if __name__ == "__main__":
	#InitClean(data)
	#grnaID('CleanImits.csv')
	AggregateExp('ImitsPared.csv')
